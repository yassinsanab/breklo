...
export default function GifToJpg() {
  ...
  function addFiles(incoming) {
    const gifs = Array.from(incoming).filter(f => f.type === 'image/gif' || f.name.toLowerCase().endsWith('.gif'));
    setFiles(prev => [...prev, ...gifs]);
  ...
  async function convert() {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        downloadBlob(blob, file.name.replace(/\.gif$/i, '.jpg'));
        await new Promise(r => setTimeout(r, 150));
      }
      setProgress(`Done — ${files.length} file${files.length > 1 ? 's' : ''} converted`);
      setDone(true);
    } catch (e) {
      alert('Error converting GIF. Please check your files.');
      setProgress('');
    }
    setLoading(false);
  }
  ...
  return (
    <ToolLayout
      title="GIF to JPG"
      subtitle="Convert GIF images to JPG format. Extracts the first frame as a high-quality JPG image."
      bullets={[
        'Converts the first frame of any GIF to JPG',
    ...
    >
      ...
      >
        ...
        <p style={{ fontWeight: 700, fontSize: 15, color: '#111', marginBottom: 4 }}>
          {files.length === 0 ? 'Drop GIF files here' : 'Add more GIFs'}
        </p>
        <p style={{ fontSize: 13, color: '#9ca3af' }}>or click to browse</p>
        <input id="fi" type="file" multiple accept=".gif,image/gif" style={{ display: 'none' }} onChange={e => addFiles(e.target.files)} />
      </div>
            ...
            {files.map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#fff', border: '1px solid #f0f0f0', borderRadius: 9, marginBottom: 6 }}>
                <div style={{ width: 36, height: 36, background: '#fefce8', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: 9, fontWeight: 800, color: '#a16207' }}>GIF</span>
                </div>
