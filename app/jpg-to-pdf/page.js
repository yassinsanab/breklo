...
export default function JpgToPdf() {
  ...
  function addFiles(incoming) {
    const imgs = Array.from(incoming).filter(f => f.type === 'image/jpeg' || f.type === 'image/jpg');
  ...
  async function convert() {
    ...
    try {
      ...
      console.error(e);
      alert('Error converting images. Make sure all files are valid JPGs.');
    }
    setLoading(false);
  }
  ...
  return (
    <ToolLayout
      ...
      bullets={[
        'Convert one or multiple JPGs at once',
    ...
    >
      ...
      >
        ...
        <p style={{ fontWeight: 700, fontSize: 15, color: '#111', marginBottom: 4 }}>
          {files.length === 0 ? 'Drop JPG files here' : 'Add more JPGs'}
        </p>
        <p style={{ fontSize: 13, color: '#9ca3af' }}>or click to browse</p>
        <input id="fi" type="file" multiple accept="image/jpeg,image/jpg" style={{ display: 'none' }} onChange={e => addFiles(e.target.files)} />
      </div>
          ...
          {files.map((f, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#fff', border: '1px solid #f0f0f0', borderRadius: 9, marginBottom: 6 }}>
              <div style={{ width: 36, height: 36, background: '#fefce8', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={{ fontSize: 9, fontWeight: 800, color: '#a16207' }}>JPG</span>
              </div>
      ...
      <button onClick={convert} disabled={files.length === 0 || loading} style={{
        ...
        {loading ? 'Converting...' : done ? 'Done! Convert again?' : `Convert ${files.length > 0 ? files.length : ''} JPG${files.length !== 1 ? 's' : ''} to PDF`}
      </button>
    </ToolLayout>
  );
}
