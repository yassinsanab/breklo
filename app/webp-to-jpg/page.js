...
export default function WebpToJpg() {
  ...
  function addFiles(incoming) {
    const webps = Array.from(incoming).filter(f =>
      f.type === 'image/webp' || f.name.toLowerCase().endsWith('.webp')
    );
    setFiles(prev => [...prev, ...webps]);
  ...
  async function convert() {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        downloadBlob(blob, file.name.replace(/\.webp$/i, '.jpg'));
        await new Promise(r => setTimeout(r, 150));
      }
      setProgress(`Done — ${files.length} file${files.length > 1 ? 's' : ''} converted`);
      setDone(true);
    } catch (e) {
      alert('Error converting WebP. Please check your files.');
      setProgress('');
    }
    setLoading(false);
  }
  ...
  return (
    <ToolLayout
      title="WebP to JPG"
      subtitle="Convert WebP images to universally compatible JPG format. Works with any software and device."
      bullets={[
    ...
    >
      ...
      >
        ...
        <p style={{ fontWeight: 700, fontSize: 15, color: '#111', marginBottom: 4 }}>
          {files.length === 0 ? 'Drop WebP files here' : 'Add more files'}
        </p>
        <p style={{ fontSize: 13, color: '#9ca3af' }}>or click to browse</p>
        <input id="fi" type="file" multiple accept=".webp,image/webp" style={{ display: 'none' }} onChange={e => addFiles(e.target.files)} />
      </div>
            ...
            {files.map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#fff', border: '1px solid #f0f0f0', borderRadius: 9, marginBottom: 6 }}>
                <div style={{ width: 36, height: 36, background: '#eff6ff', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: 9, fontWeight: 800, color: '#1d4ed8' }}>WBP</span>
                </div>
