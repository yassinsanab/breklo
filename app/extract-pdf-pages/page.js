...
export default function ExtractPages() {
  ...
  function parsePages(input, total) {
    ...
    input.split(',').forEach(part => {
      ...
      if (!isNaN(a) && !isNaN(b)) {
        for (let i = a; i <= Math.min(b, total - 1); i++) if (i >= 0) indices.add(i);
      } else if (!isNaN(a) && a >= 0 && a < total) indices.add(a);
    });
  ...
  async function extract() {
    ...
    try {
      ...
      const indices = parsePages(pages, pageCount);
      if (indices.length === 0) { alert('No valid pages found.'); setLoading(false); return; }
  ...
  return (
    <ToolLayout
      ...
      bullets={[
        'Extract any combination of pages',
    ...
    >
      ...
      {file && (
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 6 }}>
            Pages to extract {pageCount > 0 && <span style={{ color: '#9ca3af', fontWeight: 400 }}>(1–{pageCount})</span>}
          </label>
          <input
      ...
      <button onClick={extract} disabled={!file || loading || !pages.trim()} style={{
        width: '100%', padding: '13px', borderRadius: 10, border: 'none',
        background: (!file || !pages.trim()) ? '#e5e7eb' : '#0071e3',
