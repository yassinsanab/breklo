...
export default function WavToMp3() {
  ...
  async function convert() {
    ...
    try {
      setProgress('Decoding WAV audio...');
      const audioBuffer = await decodeAudioFile(file);
      setInfo({ duration: audioBuffer.duration, channels: audioBuffer.numberOfChannels });

      setProgress(`Encoding to MP3 at ${bitrate}kbps...`);
      const mp3Blob = await encodeToMp3(audioBuffer, bitrate);

      downloadBlob(mp3Blob, file.name.replace(/\.wav$/i, '') + '.mp3');
      setProgress(`Done — ${formatSize(file.size)} → ${formatSize(mp3Blob.size)}`);
      setDone(true);
    } catch (e) {
      console.error(e);
      alert('Error converting WAV file. Please check the file is valid.');
      setProgress('');
    }
    setLoading(false);
  }
  ...
  return (
    <ToolLayout
      ...
      bullets={[
        'Dramatically reduces WAV file size',
    ...
    >
        ...
        {file ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left' }}>
            <div style={{ width: 40, height: 40, background: '#f0fdf4', borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: 9, fontWeight: 800, color: '#15803d' }}>WAV</span>
            </div>
            <div style={{ flex: 1 }}>
          ...
          <>
            ...
            <p style={{ fontWeight: 700, fontSize: 15, color: '#111', marginBottom: 4 }}>Drop your WAV file here</p>
            <p style={{ fontSize: 13, color: '#9ca3af' }}>or click to browse</p>
          </>
        )}
        <input id="fi" type="file" accept=".wav,audio/wav,audio/wave" style={{ display: 'none' }} onChange={e => handleFile(e.target.files[0])} />
      </div>
      ...
      {file && (
        <div style={{ marginBottom: 14 }}>
          ...
          <p style={{ fontSize: 11, color: '#9ca3af', marginTop: 5 }}>kbps — 192 recommended for WAV source material</p>
        </div>
      )}
