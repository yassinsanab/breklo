...
export default function Mp4ToMp3() {
  ...
  async function convert() {
    ...
    try {
      ...
      setProgress(`Done — ${formatSize(mp3Blob.size)}`);
      setDone(true);
    } catch (e) {
      console.error(e);
      alert('Error converting file. Make sure it is a valid MP4/video file with audio.');
      setProgress('');
    }
    setLoading(false);
  }
    ...
    >
      ...
      {file && (
        <div style={{ marginBottom: 14 }}>
          ...
          <p style={{ fontSize: 11, color: '#9ca3af', marginTop: 5 }}>kbps — 128 is standard quality, 320 is best quality</p>
        </div>
      )}
